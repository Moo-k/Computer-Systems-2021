...
static ast parse_term()
{
    ...
    if ( have(tk_identifier) )
    {
        ...
        term = lookup_variable(t) ;
    }   else if ( have(tk_integer) )
    {
        Token t = mustbe(tk_integer) ;
        term = integer_to_ast(t) ;
    }
...
int main(int argc,char **argv)
{
    ...
    ast_print_as_xml(program,3) ;       // write the tree to standard output formatted as XML
